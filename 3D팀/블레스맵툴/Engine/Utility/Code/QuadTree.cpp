...
void Engine::CQuadTree::IsinFrustum(const VTXTEX* pTerrainVtx)
{
	...
	m_isIn = Engine::SphereInFrustum(&pTerrainVtx[m_iCenter].vPos, m_fRadius);
...
void Engine::CQuadTree::CullingToQuadTree(const VTXTEX* pTerrainVtx, INDEX32* pIndex, DWORD* pTriCnt)
{
	if(m_isIn == true)
	{
		if( LevelofDetail(pTerrainVtx) )
		{
			...
			isIn[0] = Engine::VertexInFrustum(&pTerrainVtx[m_wCorner[CORNER_LB]].vPos);
			isIn[1] = Engine::VertexInFrustum(&pTerrainVtx[m_wCorner[CORNER_RB]].vPos);
			isIn[2] = Engine::VertexInFrustum(&pTerrainVtx[m_wCorner[CORNER_RT]].vPos);
			isIn[3] = Engine::VertexInFrustum(&pTerrainVtx[m_wCorner[CORNER_LT]].vPos);
...
HRESULT Engine::CQuadTree::InitNeighbor(const VTXTEX* pTerrainVtx, CQuadTree* pParentQuad 
...
bool Engine::CQuadTree::MakeNeighbor(NEIGHBOR eNeighborID 
