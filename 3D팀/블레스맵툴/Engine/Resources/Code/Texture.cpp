...
void Engine::CTexture::Set_ConstantTable(LPD3DXEFFECT pEffect, const char* pConstantName, const _ushort& wCnt)
{
	pEffect->SetTexture(pConstantName, m_vecTexture[wCnt]);
}
...
HRESULT Engine::CTexture::LockRect(const DWORD& dwStage, D3DLOCKED_RECT* pLockedRect, CONST RECT* pRect, DWORD dwFlag)
{
	return ((LPDIRECT3DTEXTURE9)m_vecTexture[dwStage])->LockRect(0 ,pLockedRect,pRect,dwFlag);
}

HRESULT Engine::CTexture::UnLockRect(const DWORD& dwStage)
{
	return ((LPDIRECT3DTEXTURE9)m_vecTexture[dwStage])->UnlockRect(0);
}
...
LPDIRECT3DTEXTURE9* Engine::CTexture::GetTexture(const DWORD& iIndex)
{
	if(iIndex >= m_dwContainerSize)
		return NULL;
