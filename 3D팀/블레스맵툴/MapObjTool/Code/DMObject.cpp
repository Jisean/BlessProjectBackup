...
CDMObject::CDMObject(LPDIRECT3DDEVICE9 pDevice)
: CLandObject(pDevice)
, m_bCollision(false)
{
...
CDMObject::~CDMObject(void)
{

}

HRESULT CDMObject::Initialize(const TCHAR* pMeshKey, D3DXVECTOR3 vMousePos)
{
...
int CDMObject::Update(const float& fTimeDelta)
{
...
HRESULT CDMObject::AddComponent(const TCHAR* pMeshKey)
{
...
void CDMObject::Set_ContantTable(LPD3DXEFFECT	pEffect)
{
	pEffect->AddRef();
...
CDMObject* CDMObject::Create(LPDIRECT3DDEVICE9 pDevice, const TCHAR* pMeshKey, D3DXVECTOR3 vMousePos)
{
	CDMObject*		pGameObject = new CDMObject(pDevice);
