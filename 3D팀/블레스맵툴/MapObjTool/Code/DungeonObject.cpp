...
HRESULT CDungeonObject::Initialize(const TCHAR* pMeshKey, D3DXVECTOR3 vMousePos)
{
	_tcscpy_s(m_tcKey,pMeshKey);

	FAILED_CHECK(AddComponent(pMeshKey));

	m_pInfo->m_vScale = D3DXVECTOR3(0.015f, 0.015f, 0.015f);
...
int CDungeonObject::Update(const float& fTimeDelta)
{
...
void CDungeonObject::Set_ContantTable(LPD3DXEFFECT	pEffect)
{
	pEffect->AddRef();
...
CDungeonObject* CDungeonObject::Create(LPDIRECT3DDEVICE9 pDevice, const TCHAR* pMeshKey, D3DXVECTOR3 vMousePos)
{
