...
bool CMouseCol::PickNaviCell(D3DXVECTOR3* pOut, Engine::CNaviCell* pCell, int* pOutIndex)
{
	Translation_ViewSpace();
	D3DXMATRIX matIdentity;
	D3DXMatrixIdentity(&matIdentity);
	...
	int iSize = Engine::Get_Cell().size();
...
void CMouseCol::PickSetTerrain(D3DXVECTOR3* pOut, Engine::VTXTEX* pTerrainVtx, LPDIRECT3DVERTEXBUFFER9* pVB, bool bUpDown, float fRange)
{
	Translation_ViewSpace();
	...
	for(int z = 0; z < VTXCNTZ - 1; ++z)
	{
		for(int x = 0; x < VTXCNTX - 1; ++x)
		{
			...
			if(D3DXIntersectTri(&pVertex[iIndex + VTXCNTX + 1].vPos, 
				...
				&fU, &fV, &fDist))
			{
				/*&pVertex[iIndex + VTXCNTX + 1].vPos;	너는 오른쪽 위다
				pVertex[iIndex + VTXCNTX].vPos			너는 왼쪽 위다	
					&pVertex[iIndex + 1].vPos			너는 아래 오른쪽이다
					pVertex[iIndex].vPos*/		      // 너는 아래 왼쪽이다			
			...
			if(D3DXIntersectTri(&pVertex[iIndex].vPos, 
				...
				&fU, &fV, &fDist))
			{
				/*&pVertex[iIndex + VTXCNTX + 1].vPos;
				pVertex[iIndex + VTXCNTX].vPos
					&pVertex[iIndex + 1].vPos
					pVertex[iIndex].vPos*/

				/*float fX = pVertex[iIndex + VTXCNTX + 1].vPos.x - ((pVertex[iIndex + VTXCNTX + 1].vPos.x - pVertex[iIndex + VTXCNTX].vPos.x) / 2.f);
				float fZ = pVertex[iIndex + VTXCNTX + 1].vPos.z - ((pVertex[iIndex + VTXCNTX + 1].vPos.z - pVertex[iIndex + 1].vPos.z) / 2.f);
				float fY = 1.f;*/
