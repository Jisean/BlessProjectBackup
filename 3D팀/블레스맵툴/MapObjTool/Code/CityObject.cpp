...
HRESULT CCityObject::Initialize(const TCHAR* pMeshKey, D3DXVECTOR3 vMousePos)
{
	...
	TCHAR temp[8];
	ZeroMemory(&temp,8);
	_tcsncpy_s(temp, pMeshKey, 7);
	temp[7] = '\0';

	BOOL b = _tcscmp(temp,L"Village");

	if(!b)
	{
		m_pInfo->m_vScale.x = 1.f;
		m_pInfo->m_vScale.y = 1.f;
		m_pInfo->m_vScale.z = 1.f;

	}
	else
	{
		m_pInfo->m_vScale.x = 0.005f;
		m_pInfo->m_vScale.y = 0.005f;
		m_pInfo->m_vScale.z = 0.005f;
	}
...
int CCityObject::Update(const float& fTimeDelta)
{
...
void CCityObject::Render(void)
{
	...
	if( pObjTool->m_ColDisvisable.GetCheck() == FALSE)
	{
...
void CCityObject::Set_ContantTable(LPD3DXEFFECT	pEffect)
{
	pEffect->AddRef();
...
CCityObject* CCityObject::Create(LPDIRECT3DDEVICE9 pDevice, const TCHAR* pMeshKey, D3DXVECTOR3 vMousePos)
{
