...
void CNode::Init_Node(const _vec3* pVtxPos, const _ulong& dwLT, const _ulong& dwRT, const _ulong& dwLB, const _ulong& dwRB)
{	
	...
	_ulong		dwLC, dwTC, dwRC, dwBC, dwCenter;
	dwLC = (dwLT + dwLB) >> 1;
	dwTC = (dwLT + dwRT) >> 1;
	dwRC = (dwRT + dwRB) >> 1;
	dwBC = (dwLB + dwRB) >> 1;
...
void CNode::Decision_In(LPDIRECT3DDEVICE9 pGraphicDev, CFrustum* pFrustum, INDEX32* pIndex, int* pTriCnt)
{	
	//if(NULL == m_pChildNode[NODE_LT])
	if(LevelofDetail(pGraphicDev)
	...
	{	
		...
		_bool	isNeighborDraw[4] = {true, true, true, true};

		if(m_dwNeighborCenter[CENTER_TOP] != MAX)
			isNeighborDraw[0] = LevelofDetail(pGraphicDev, m_dwNeighborCenter[CENTER_TOP]);
		if(m_dwNeighborCenter[CENTER_RIGHT] != MAX)
			isNeighborDraw[1] = LevelofDetail(pGraphicDev, m_dwNeighborCenter[CENTER_RIGHT]);
		if(m_dwNeighborCenter[CENTER_LEFT] != MAX)
			isNeighborDraw[2] = LevelofDetail(pGraphicDev, m_dwNeighborCenter[CENTER_LEFT]);
		if(m_dwNeighborCenter[CENTER_BOTTOM] != MAX)
			isNeighborDraw[3] = LevelofDetail(pGraphicDev, m_dwNeighborCenter[CENTER_BOTTOM]);
		...
		{
			bIn[0] = pFrustum->Decision_In(pGraphicDev, &m_pVtxPos[m_dwCorner[CORNER_LT]]);
			bIn[1] = pFrustum->Decision_In(pGraphicDev, &m_pVtxPos[m_dwCorner[CORNER_RT]]);
			bIn[2] = pFrustum->Decision_In(pGraphicDev, &m_pVtxPos[m_dwCorner[CORNER_RB]]);
			...
			bIn[0] = pFrustum->Decision_In(pGraphicDev, &m_pVtxPos[m_dwCorner[CORNER_LT]]);
			bIn[1] = pFrustum->Decision_In(pGraphicDev, &m_pVtxPos[m_dwCorner[CORNER_RB]]);
			bIn[2] = pFrustum->Decision_In(pGraphicDev, &m_pVtxPos[m_dwCorner[CORNER_LB]]);
	...
	if(true == pFrustum->Decision_In(pGraphicDev, &m_pVtxPos[m_dwCenter], m_fRadius))
	{
		for (_ulong i = 0; i < NODE_END; ++i)
		{
			if(NULL != m_pChildNode[i])
				m_pChildNode[i]->Decision_In(pGraphicDev, pFrustum, pIndex, pTriCnt);
		}
	}
}

void CNode::SetUp_NeighborCenterIdx(const _ulong& dwVtxCntX, const _ulong& dwVtxCntZ)
{
...
CNode* CNode::Create(const _vec3* pVtxPos, const _ulong& dwLT, const _ulong& dwRT, const _ulong& dwLB, const _ulong& dwRB)
{
...
_bool CNode::LevelofDetail(LPDIRECT3DDEVICE9 pGraphicDev)
{
	_matrix			matView;
...
_bool CNode::LevelofDetail(LPDIRECT3DDEVICE9 pGraphicDev, const _ulong& dwCenter)
{
	_matrix			matView;
	pGraphicDev->GetTransform(D3DTS_VIEW, &matView);
	D3DXMatrixInverse(&matView, NULL, &matView);
...
void CNode::Compute_NeighborCenter(const _ulong& dwVtxCntX, const _ulong& dwVtxCntZ)
{
	_ulong			dwNeighborIndex = 0;
...
void CNode::Free(void)
{
	...
	if(0 == dwRefCnt)
	{
		for (_ulong i = 0; i < NODE_END; ++i)
		{
			Engine::safe_release(m_pChildNode[i]);
		}
		delete this;
	}
