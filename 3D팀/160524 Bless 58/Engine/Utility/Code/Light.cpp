...
HRESULT Engine::CLight::Init_Light(const D3DLIGHT9* pLightInfo)
{
...
Engine::CLight* Engine::CLight::Create(LPDIRECT3DDEVICE9 pDevice , const D3DLIGHT9* pLightInfo)
{
	...
	if(FAILED(pLight->Init_Light(pLightInfo)))
...
D3DLIGHT9* Engine::CLight::Set_LightInfo(void)
{
	return &m_LightInfo;
}

bool* Engine::CLight::Get_LightRender(void)
{
	return &m_bRender;
}