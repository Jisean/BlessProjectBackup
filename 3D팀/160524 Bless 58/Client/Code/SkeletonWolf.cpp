...
CSkeletonWolf::CSkeletonWolf(LPDIRECT3DDEVICE9 pDevice)
: CMonsters(pDevice)
, m_bPush(false)
//, m_iAniIdx(0)
{
...
CSkeletonWolf::~CSkeletonWolf(void)
{

}

HRESULT CSkeletonWolf::Initialize(void)
{
...
_int CSkeletonWolf::Update(const _float& fTimeDelta)
{
...
void CSkeletonWolf::Render(void)
{
...
void CSkeletonWolf::AniFrameEnd(Engine::ANI_INFO* pData)
{
...
HRESULT CSkeletonWolf::AddComponent(void)
{
	...
	//StaticMesh
	pComponent = Engine::Clone_Resource(RESOURCE_STAGE, L"Mesh_SkeletonWolf");
...
void CSkeletonWolf::Set_ContantTable(LPD3DXEFFECT pEffect)
{
...
CSkeletonWolf* CSkeletonWolf::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CSkeletonWolf*		pGameObject = new CSkeletonWolf(pDevice);
...
void CSkeletonWolf::KeyCheck(void)
{

}

void CSkeletonWolf::Sound(DWORD dwMyIndex)
{
	...
	case CAIMgr::STATE_IDLE_P:
		if((float)m_pMesh->GetTrackPosition() > 0.f && (float)m_pMesh->GetTrackPosition() < 0.1f)
			CSoundMgr::GetInstance()->MummySound(L"SkelWolfIdle.ogg", dwMyIndex, fSoundVolume);
		break;
	...
	case CAIMgr::STATE_ATK_1:
		if((float)m_pMesh->GetTrackPosition() > 0.f && (float)m_pMesh->GetTrackPosition() < 0.1f)
			CSoundMgr::GetInstance()->SkelWolfSound(L"SkelWolfAtt.ogg", dwMyIndex, fSoundVolume);
		break;
	...
	case CAIMgr::STATE_DIE:
		if((float)m_pMesh->GetTrackPosition() > 0.f && (float)m_pMesh->GetTrackPosition() < 0.1f)
			CSoundMgr::GetInstance()->SkelWolfSound(L"SkelWolfDead.ogg", dwMyIndex, fSoundVolume);
		break;
	}
