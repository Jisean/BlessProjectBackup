...
CMainQStart::CMainQStart(LPDIRECT3DDEVICE9 pDevice)
: CUI(pDevice)
, m_fCurTime(0.f)
, m_fMaxTime(3.f)
, m_bNewQuest(false)
...
CMainQStart::~CMainQStart(void)
{

}

HRESULT CMainQStart::Initialize(void)
{
...
_int CMainQStart::Update(const _float& fTimeDelta)
{
...
void CMainQStart::Free(void)
{
	CUI::Free();
}

HRESULT CMainQStart::AddComponent(void)
{
...
CMainQStart* CMainQStart::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CMainQStart*		pGameObject = new CMainQStart(pDevice);
...
void CMainQStart::Set_TextureInit(eComplete eTexNum, float fX, float fY, float fSizeX, float fSizeY)
{
	m_fX[eTexNum] = fX;
	m_fY[eTexNum] = fY;
...
void CMainQStart::Set_TexView(void)
{
	for(int i = 0; i < COMPLETE_END; ++i)
	{
...
void CMainQStart::Init_MoveTex(eComplete eTexNum)
{
...
void CMainQStart::Move_Tex(void)
{
	float fTemp2 = 0.001f;
