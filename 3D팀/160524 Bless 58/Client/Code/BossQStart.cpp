...
CBossQStart::CBossQStart(LPDIRECT3DDEVICE9 pDevice)
: CUI(pDevice)
, m_fCurTime(0.f)
, m_fMaxTime(3.f)
, m_bNewQuest(false)
...
CBossQStart::~CBossQStart(void)
{

}

HRESULT CBossQStart::Initialize(void)
{
...
_int CBossQStart::Update(const _float& fTimeDelta)
{
	if(((CMoveQComplete*)(Engine::Get_ObjList(LAYER_UI, L"MoveQComplete")->front()))->Get_NewQuest() == false)
		return 0;
...
void CBossQStart::Free(void)
{
	CUI::Free();
}

HRESULT CBossQStart::AddComponent(void)
{
...
CBossQStart* CBossQStart::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CBossQStart*		pGameObject = new CBossQStart(pDevice);
...
void CBossQStart::Set_TextureInit(eComplete eTexNum, float fX, float fY, float fSizeX, float fSizeY)
{
	m_fX[eTexNum] = fX;
	m_fY[eTexNum] = fY;
...
void CBossQStart::Set_TexView(void)
{
	for(int i = 0; i < COMPLETE_END; ++i)
	{
...
void CBossQStart::Init_MoveTex(eComplete eTexNum)
{
...
void CBossQStart::Move_Tex(void)
{
	float fTemp2 = 0.001f;
