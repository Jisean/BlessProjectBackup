...
CWyvernTwins::CWyvernTwins(LPDIRECT3DDEVICE9 pDevice)
: CWyverns(pDevice)
{

}
CWyvernTwins::~CWyvernTwins(void)
{

}
HRESULT CWyvernTwins::Initialize(void)
{
...
_int CWyvernTwins::Update(const _float& fTimeDelta)
{
...
void CWyvernTwins::UpdateAnimation(CWyverns::WYVERNANI eTrack ,float fTrackTime)
{
	m_eWYTrack = eTrack;
...
////////////////////////////////////////////////////////////////////////// 애니메이션 관련 함수
void CWyvernTwins::Ani_Jump_To_Wyvern(float fTrackTime)
{
	...
	if(m_eWYTrack == WY_RUN_JUMP_UP)
	{
		if(m_fSoundTime < 0.1f)
			CSoundMgr::GetInstance()->WyvernSound3(L"WyvernFlyMove.ogg",fSoundVolume);
		if(m_fSoundTime > 6.f)
			m_fSoundTime = 0.f;
...
void CWyvernTwins::Ani_Move_To_Wyvern(float fTrackTime)
{
	fTrackTime = (float)m_pWyvernMesh->GetTrackPosition();
	m_pWyvernMesh->SetAnimationSet(m_eWYTrack);
...
void CWyvernTwins::Ani_Fly_To_Wyvern(float fTrackTime)
{
	fTrackTime = (float)m_pWyvernMesh->GetTrackPosition();
	m_pWyvernMesh->SetAnimationSet(m_eWYTrack);
	...
	if(m_eWYTrack == CWyverns::WY_FLYING_IDLE)
	{
		...
		{
			if(m_fSoundTime < 0.1f)
				CSoundMgr::GetInstance()->WyvernSound3(L"WyvernFlyIdle.ogg",fSoundVolume);
			if(m_fSoundTime > 1.35f)
				m_fSoundTime = 0.f;
...
void CWyvernTwins::Ani_Stand_To_Wyvern(float fTrackTime)
{
	fTrackTime = (float)m_pWyvernMesh->GetTrackPosition();
	m_pWyvernMesh->SetAnimationSet(m_eWYTrack);
...
//////////////////////////////////////////////////////////////////////////
HRESULT CWyvernTwins::AddComponent(void)
{
	...
	//StaticMesh
	pComponent = Engine::Clone_Resource(RESOURCE_REMAIN, L"Mesh_WyvernTwins");
...
void CWyvernTwins::Set_ContantTable(LPD3DXEFFECT	pEffect)
{
...
CWyvernTwins* CWyvernTwins::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CWyvernTwins*		pGameObject = new CWyvernTwins(pDevice);
