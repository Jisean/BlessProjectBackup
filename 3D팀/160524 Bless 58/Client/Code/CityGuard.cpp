...
CCityGuard::CCityGuard(LPDIRECT3DDEVICE9 pDevice)
: CNPCs(pDevice)
{

}

CCityGuard::~CCityGuard(void)
{

}

HRESULT CCityGuard::Initialize(void)
{
...
_int CCityGuard::Update(const _float& fTimeDelta)
{
	if(!m_bIninInfo)
	{
...
void CCityGuard::AniFrameEnd(/*Engine::ANI_INFO* pData*/)
{
...
HRESULT CCityGuard::AddComponent(void)
{
	...
	//StaticMesh
	pComponent = Engine::Clone_Resource(RESOURCE_STAGE, L"CityGuard");
...
void CCityGuard::Set_ContantTable(LPD3DXEFFECT pEffect)
{	
...
CCityGuard* CCityGuard::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CCityGuard*		pGameObject = new CCityGuard(pDevice);
