...
CQuestComplete1::CQuestComplete1(LPDIRECT3DDEVICE9 pDevice)
: CUI(pDevice)
, m_fCurTime(0.f)
, m_fMaxTime(3.f)
, m_bNewQuest(false)
...
CQuestComplete1::~CQuestComplete1(void)
{

}

HRESULT CQuestComplete1::Initialize(void)
{
...
_int CQuestComplete1::Update(const _float& fTimeDelta)
{
	CMainPlayer* m_pMainPlayer = (CMainPlayer*)((CPlayers*)Engine::Get_ObjList(LAYER_GAMELOGIC,L"Player")->front());

	if(	*m_pMainPlayer->Get_QuestStep() != CMainPlayer::QUEST_COMPLETE || 
		*m_pMainPlayer->Get_QuestType() != CMainPlayer::QUESTTYPE_MAIN)
		return 0;
	else if (m_bEndQuest == false)
	{
		CSoundMgr::GetInstance()->PlayQuestSound(L"FinalQuestEnd.ogg");
		m_bEndQuest = true;
	}
	...
	float fOldPower = ((COption*)(Engine::Get_ObjList(LAYER_UI, L"Option")->front()))->Get_OldLightPower();
	float fCurPower = ((CQuestNpc*)(Engine::Get_ObjList(LAYER_UI, L"QuestNpc")->front()))->Get_LightPower();

	if(!m_bNewQuest)
	{
		// 5초동안 복구..
		float fTemp = ((2.f - fCurPower) * (m_fCurTime / m_fMaxTime));

		if(fCurPower + fTemp <= 2.f)
		{
			*(Engine::Set_RendererLightPower()) = fCurPower + fTemp;
			((COption*)(Engine::Get_ObjList(LAYER_UI, L"Option")->front()))->Adjust_Light();
		}
		...
		fAmbient += 0.4f * (m_fCurTime / m_fMaxTime);
		pLight->Set_LightInfo()->Ambient = D3DXCOLOR(fAmbient, fAmbient, fAmbient, 1.f);
	}
	...
	{
		if(m_bNewQuest == false)
			CSoundMgr::GetInstance()->PlayQuestSound(L"FirstQuestAccept.ogg");
		m_bNewQuest = true;
		((COption*)(Engine::Get_ObjList(LAYER_UI, L"Option")->front()))->Set_OldLightPower(2.f);
	}
...
void CQuestComplete1::Render(void)
{
...
void CQuestComplete1::Free(void)
{
	CUI::Free();
}

HRESULT CQuestComplete1::AddComponent(void)
{
	...
	pComponent = Engine::Clone_Resource(RESOURCE_UI, L"Texture_Quest_MainQuest_CompleteMainQ");
	m_pTexture[COMPLETE_TXT] = dynamic_cast<Engine::CTexture*>(pComponent);
	NULL_CHECK_RETURN(m_pTexture[COMPLETE_TXT], E_FAIL);
	m_htComponent.Insert_Table(L"Texture_Quest_MainQuest_CompleteMainQ", pComponent);


	return S_OK;
}

CQuestComplete1* CQuestComplete1::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CQuestComplete1*		pGameObject = new CQuestComplete1(pDevice);
...
void CQuestComplete1::Set_TextureInit(eComplete eTexNum, float fX, float fY, float fSizeX, float fSizeY)
{
	m_fX[eTexNum] = fX;
	m_fY[eTexNum] = fY;
...
void CQuestComplete1::Set_TexView(void)
{
	for(int i = 0; i < COMPLETE_END; ++i)
	{
...
void CQuestComplete1::Init_MoveTex(eComplete eTexNum)
{
...
void CQuestComplete1::Move_Tex(void)
{
	float fTemp2 = 0.001f;
