...
CNPC_Shield::CNPC_Shield(LPDIRECT3DDEVICE9 pDevice)
: CEquipment(pDevice)
{
	m_pNpc = NULL;
	m_pNPCCuard2 = NULL;
...
CNPC_Shield::~CNPC_Shield(void)
{

}

HRESULT CNPC_Shield::Initialize(void)
{
...
_int CNPC_Shield::Update(const _float& fTimeDelta)
{
...
HRESULT CNPC_Shield::AddComponent(void)
{
	...
	//StaticMesh
	pComponent = Engine::Clone_Resource(RESOURCE_REMAIN, L"Mesh_Shield5");
...
CNPC_Shield* CNPC_Shield::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CNPC_Shield*		pGameObject = new CNPC_Shield(pDevice);
