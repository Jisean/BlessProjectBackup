...
CWyvernLand::CWyvernLand(LPDIRECT3DDEVICE9 pDevice)
: CWyverns(pDevice)
{

}
CWyvernLand::~CWyvernLand(void)
{

}
HRESULT CWyvernLand::Initialize(void)
{
...
_int CWyvernLand::Update(const _float& fTimeDelta)
{
	...
	UpdateAnimation(m_eWYTrack,m_fTimeDelta);

	Engine::Add_RenderGroup(Engine::CRenderer::TYPE_NONEALPHA, this);

	return iResult;
}
void CWyvernLand::Render(void)
{
	UpdateAnimation(m_eWYTrack,m_fTimeDelta);
...
void CWyvernLand::UpdateAnimation(CWyverns::WYVERNANI eTrack ,float fTrackTime)
{
	m_eWYTrack = eTrack;
	...
	m_pWyvernMesh->FrameMove((fTrackTime * 1.2f) / m_fSlowTime);
...
////////////////////////////////////////////////////////////////////////// 애니메이션 관련 함수
void CWyvernLand::Ani_Jump_To_Wyvern(float fTrackTime)
{
	...
	if(m_eWYTrack == WY_RUN_JUMP_UP)
	{
		...
		if(m_fSoundTime > 6.f)
			m_fSoundTime = 0.f;
		m_fJumpAcc += 1.5f * m_fTimeDelta;
		m_pInfo->m_vPos.y += (0.98f * m_fJumpAcc + m_fJumpAcc * 0.3f - 0.3f) * m_fTimeDelta;
	}
	if(m_eWYTrack == WY_JUMP_LAND)
	{
...
void CWyvernLand::Ani_Move_To_Wyvern(float fTrackTime)
{
	fTrackTime = (float)m_pWyvernMesh->GetTrackPosition();
	m_pWyvernMesh->SetAnimationSet(m_eWYTrack);
...
void CWyvernLand::Ani_Fly_To_Wyvern(float fTrackTime)
{
...
void CWyvernLand::Ani_Stand_To_Wyvern(float fTrackTime)
{
	fTrackTime = (float)m_pWyvernMesh->GetTrackPosition();
	m_pWyvernMesh->SetAnimationSet(m_eWYTrack);
...
//////////////////////////////////////////////////////////////////////////
HRESULT CWyvernLand::AddComponent(void)
{
	...
	//StaticMesh
	pComponent = Engine::Clone_Resource(RESOURCE_REMAIN, L"Mesh_WyvernLand");
...
void CWyvernLand::Set_ContantTable(LPD3DXEFFECT	pEffect)
{
...
CWyvernLand* CWyvernLand::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CWyvernLand*		pGameObject = new CWyvernLand(pDevice);
