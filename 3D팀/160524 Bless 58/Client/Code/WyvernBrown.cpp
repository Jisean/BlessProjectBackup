...
CWyvernBrown::CWyvernBrown(LPDIRECT3DDEVICE9 pDevice)
: CWyverns(pDevice)
{

}
CWyvernBrown::~CWyvernBrown(void)
{

}
HRESULT CWyvernBrown::Initialize(void)
{
...
_int CWyvernBrown::Update(const _float& fTimeDelta)
{
...
void CWyvernBrown::UpdateAnimation(CWyverns::WYVERNANI eTrack ,float fTrackTime)
{
	m_eWYTrack = eTrack;
...
////////////////////////////////////////////////////////////////////////// 애니메이션 관련 함수
void CWyvernBrown::Ani_Jump_To_Wyvern(float fTrackTime)
{
	...
	if(m_eWYTrack == WY_RUN_JUMP_UP)
	{
		if(m_fSoundTime < 0.1f)
			CSoundMgr::GetInstance()->WyvernSound2(L"WyvernFlyMove.ogg",fSoundVolume);
		if(m_fSoundTime > 6.f)
			m_fSoundTime = 0.f;
...
void CWyvernBrown::Ani_Move_To_Wyvern(float fTrackTime)
{
	fTrackTime = (float)m_pWyvernMesh->GetTrackPosition();
	m_pWyvernMesh->SetAnimationSet(m_eWYTrack);
...
void CWyvernBrown::Ani_Fly_To_Wyvern(float fTrackTime)
{
	fTrackTime = (float)m_pWyvernMesh->GetTrackPosition();
	m_pWyvernMesh->SetAnimationSet(m_eWYTrack);
	...
	if(m_eWYTrack == CWyverns::WY_FLYING_IDLE)
	{
		...
		{
			if(m_fSoundTime < 0.1f)
				CSoundMgr::GetInstance()->WyvernSound2(L"WyvernFlyIdle.ogg",fSoundVolume);
			if(m_fSoundTime > 1.0f)
				m_fSoundTime = 0.f;
...
void CWyvernBrown::Ani_Stand_To_Wyvern(float fTrackTime)
{
	fTrackTime = (float)m_pWyvernMesh->GetTrackPosition();
	m_pWyvernMesh->SetAnimationSet(m_eWYTrack);
...
//////////////////////////////////////////////////////////////////////////
HRESULT CWyvernBrown::AddComponent(void)
{
	...
	//StaticMesh
	pComponent = Engine::Clone_Resource(RESOURCE_REMAIN, L"Mesh_WyvernBrown");
...
void CWyvernBrown::Set_ContantTable(LPD3DXEFFECT	pEffect)
{
...
CWyvernBrown* CWyvernBrown::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CWyvernBrown*		pGameObject = new CWyvernBrown(pDevice);
