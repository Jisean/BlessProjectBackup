...
CBaseTerrain::CBaseTerrain(LPDIRECT3DDEVICE9 pDevice)
...
HRESULT CBaseTerrain::Initialize(void)
{
	FAILED_CHECK(AddComponent());

	m_pTerrainVtx = new Engine::VTXTEX[4];
	m_pIndex = new Engine::INDEX32[2];
...
_int CBaseTerrain::Update(const _float& fTimeDelta)
{
...
HRESULT CBaseTerrain::AddComponent(void)
{
	...
	//Buffer
	pComponent = Engine::Clone_Resource(RESOURCE_STAGE, L"Buffer_BaseTerrainTex");
...
CBaseTerrain* CBaseTerrain::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CBaseTerrain*		pGameObject = new CBaseTerrain(pDevice);
...
void CBaseTerrain::Culling_Frustum(void)
{
...
void CBaseTerrain::Set_ConstantTable(LPD3DXEFFECT pEffect)
{
...
void CBaseTerrain::Set_ShadowCheck(LPD3DXEFFECT pEffect, D3DXMATRIX* pWorldMatrix)
{
...
void CBaseTerrain::LoadTerrain(void)
{
	int iCntX = 2;
	int iCntZ = 2;
	int iItv = 512;
...
void CBaseTerrain::SetNormal(void)
{
	int		iIndex = 0;
	int iTriCnt = 0;
	for(int z = 0; z < 1; ++z)
	{
		for(int x = 0; x < 1; ++x)
		{
			...
			m_pIndex[iTriCnt]._1 = iIndex + 2;
			m_pIndex[iTriCnt]._2 = iIndex + 2 + 1;
			m_pIndex[iTriCnt]._3 = iIndex + 1;
