...
CBossQComplete::CBossQComplete(LPDIRECT3DDEVICE9 pDevice)
: CUI(pDevice)
, m_fCurTime(0.f)
, m_fMaxTime(3.f)
, m_bNewQuest(false)
...
CBossQComplete::~CBossQComplete(void)
{

}

HRESULT CBossQComplete::Initialize(void)
{
...
void CBossQComplete::Render(void)
{
...
void CBossQComplete::Free(void)
{
	CUI::Free();
}

HRESULT CBossQComplete::AddComponent(void)
{
	...
	pComponent = Engine::Clone_Resource(RESOURCE_UI, L"Texture_Quest_BossWidow_BossCompleteTxt");
	m_pTexture[COMPLETE_TXT] = dynamic_cast<Engine::CTexture*>(pComponent);
	NULL_CHECK_RETURN(m_pTexture[COMPLETE_TXT], E_FAIL);
	m_htComponent.Insert_Table(L"Texture_Quest_BossWidow_BossCompleteTxt", pComponent);


	return S_OK;
}

CBossQComplete* CBossQComplete::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CBossQComplete*		pGameObject = new CBossQComplete(pDevice);
...
void CBossQComplete::Set_TextureInit(eComplete eTexNum, float fX, float fY, float fSizeX, float fSizeY)
{
	m_fX[eTexNum] = fX;
	m_fY[eTexNum] = fY;
...
void CBossQComplete::Set_TexView(void)
{
	for(int i = 0; i < COMPLETE_END; ++i)
	{
...
void CBossQComplete::Init_MoveTex(eComplete eTexNum)
{
...
void CBossQComplete::Move_Tex(void)
{
	float fTemp2 = 0.001f;
