...
HRESULT CRanger::Initialize(void)
{
	...
	m_fSkillCooltime_Max[SKILL_1] =	13.f;		/// 곡사
	m_fSkillCooltime_Max[SKILL_2] = 11.f;		/// 난사
	m_fSkillCooltime_Max[SKILL_3] = 8.f;		/// 정조준
	m_fSkillCooltime_Max[SKILL_4] =	6.f;		/// 무브샷
	m_fSkillCooltime_Max[SKILL_5] =	7.f;		/// 슬로우 or 스턴샷
	m_fSkillCooltime_Max[SKILL_DASH] =  10.f;	/// 백샷
	m_fSkillCooltime_Max[SKILL_INNATE] = 10.f;	/// 수류탄..
...
{	
	...
	if(m_bCulled)
	{
		...
		pEffect->SetFloat("g_fSpecPow", 0.2f);		// 파워 설정 값이 높을 수록 빛의 세기가 세진다.
...
void CRanger::AI_Combat()
{
	...
	if(m_fCurrentDis <= m_fTargetDis)
	{
		...
		{
			if(!m_bCoolTimeSet.test(SKILL_1))	/// 곡사
			{
				if(m_bTrack == true)
					return;
		...
		if(m_fCurrentDis < 2.5f)
		{
			...
			if(!m_bSkillSet.test(SKILL_1))
			{
				...
				if(!m_bCoolTimeSet.test(SKILL_3))	/// 정조준
				{
				...
				if(!m_bCoolTimeSet.test(SKILL_2))	/// 난사
				{
		...
		if(fTrackTime >= 1.f && fTrackTime <= 6.f)
		{
			if(m_pTargetMonster != NULL)
			{
				...
				if(m_bCancel == false)
				{
					if(m_pTargetMonster != NULL)
					{	
						Engine::Call_RangerFX(Engine::R_SKILL1,&vPos,false);	/// 이펙트 랜더
						if(m_bMainPlayer)
						{
