...
CMoveQStart::CMoveQStart(LPDIRECT3DDEVICE9 pDevice)
: CUI(pDevice)
, m_fCurTime(0.f)
, m_fMaxTime(3.f)
, m_bNewQuest(false)
...
CMoveQStart::~CMoveQStart(void)
{

}

HRESULT CMoveQStart::Initialize(void)
{
...
_int CMoveQStart::Update(const _float& fTimeDelta)
{

	CMainPlayer* m_pMainPlayer = (CMainPlayer*)((CPlayers*)Engine::Get_ObjList(LAYER_GAMELOGIC,L"Player")->front());

	if(((CQuestComplete1*)(Engine::Get_ObjList(LAYER_UI, L"QuestComplete1")->front()))->Get_NewQuest() == false)
		return 0;
	//Move_Tex();
...
void CMoveQStart::Free(void)
{
	CUI::Free();
}

HRESULT CMoveQStart::AddComponent(void)
{
	...
	pComponent = Engine::Clone_Resource(RESOURCE_UI, L"Texture_Quest_Complete_Mark");
	m_pTexture[COMPLETE_MARK] = dynamic_cast<Engine::CTexture*>(pComponent);
	NULL_CHECK_RETURN(m_pTexture[COMPLETE_MARK], E_FAIL);
	m_htComponent.Insert_Table(L"Texture_Quest_Complete_Mark", pComponent);
...
CMoveQStart* CMoveQStart::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CMoveQStart*		pGameObject = new CMoveQStart(pDevice);
...
void CMoveQStart::Set_TextureInit(eComplete eTexNum, float fX, float fY, float fSizeX, float fSizeY)
{
	m_fX[eTexNum] = fX;
	m_fY[eTexNum] = fY;
...
void CMoveQStart::Set_TexView(void)
{
	for(int i = 0; i < COMPLETE_END; ++i)
	{
...
void CMoveQStart::Init_MoveTex(eComplete eTexNum)
{
...
void CMoveQStart::Move_Tex(void)
{
	float fTemp2 = 0.001f;
